/* ==========================================
   KasirKu — Customers Admin Module
   Dipakai di index.html (dashboard admin)
   ========================================== */
window.KR = window.KR || {};
...
(function () {
    ...
    if (statsEl) {
      statsEl.innerHTML = `
        ...
        <div class="order-stat">
          <div class="order-stat-label">Total Order</div>
          <div class="order-stat-value">${totalOrders}</div>
        </div>`;
    }

    let filtered = customers;
    if (filterType === 'registered') filtered = customers.filter(c => !c.is_guest);
    if (filterType === 'guest') filtered = customers.filter(c => c.is_guest);

    if (!filtered.length) {
      listEl.innerHTML = `<div class="empty-state">
        <i data-lucide="users"></i>
        <h3>Belum ada customer</h3>
        <p>Customer yang login / order di toko kamu akan muncul di sini</p>
      </div>`;
      if (window.lucide) lucide.createIcons();
      return;
    }
  ...
  // Auto-load saat app ready + login
  window.addEventListener('kasirku:ready', () => {
    if (KR.auth.isLoggedIn()) {
      // Defer supaya tidak nabrak init utama
      setTimeout(loadCustomers, 800);
    }
  });

  // Re-load setelah login (event baru)
  window.addEventListener('customer:refresh', () => {
    if (KR.auth.isLoggedIn()) loadCustomers();
  });
})();
